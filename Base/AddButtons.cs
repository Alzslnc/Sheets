    ...
    internal static class StartEvents
    {
        ...
        private static List<(string, List<(string, string, string)>)> commands = new List<(string, List<(string, string, string)>)>()
        {
             ...
             { 
                 ...
                 ("Sheets_Delete", "Удалить схемы", "Удаляет блоки листов из таблицы блоков, опционально вместе с вставками схем"),
                 ("Layout_Create", "Создать листы", "Создает листы по образцу в области модели"),
                 ...
                 ("LayoutToBlock", "Пронумеровать штампы", "Переносит номер листа в соответствующий атрибут блока")
             })
        };
        /// <summary>
        /// Инициализация
