...
namespace Sheets.Program
{
    internal static class SheetsCreateClass
    {
        ...
        {
            //создаем слой фона
            Support.CreateLayer(AppConstants.BackgroundSheetsLayer, true, tr);
            //создаем слой фона
            Support.CreateLayer(AppConstants.HeaderLayer, true, tr);
            //создаем слой фона
            Support.CreateLayer(AppConstants.BorderLayer, true, tr);
            //создаем слой фона
            Support.CreateLayer(AppConstants.SheetsLayer, true, tr);
            //создаем слой фона
            Support.CreateLayer(AppConstants.CurrentObjectsLayer, true, tr);
        }    
        ...
        private static BlockTableRecord CreateBlock(Point3d blockOriginPosition, double blockScale, Transaction tr, BlockTable bt, List<ViewportData> viewports, out List<BlockReference> references)
        {
            ...
            string name = $"{AppConstants.BackgroundSheetsLayer}({Settings.Default.ViewportLayerName})";
            if (bt.Has(name))
            {
            ...
            foreach (ViewportData data in viewports)
            {
                ...
                curve.Layer = AppConstants.SheetsLayer;
            ...
            if (!Settings.Default.SelectPosition || Settings.Default.PositionType != PositionType.viewport)
            {
                ...
                polyline.Layer = AppConstants.BorderLayer;
            ...
            { 
                ...
                Layer = AppConstants.HeaderLayer, ColorIndex = 256, 
