    ...
    internal class ExtentsCreateClass
    {
        internal static void Create()
        {
            ...
            using (Transaction tr = HostApplicationServices.WorkingDatabase.TransactionManager.StartTransaction())
            {
                ...
                string name = $"{AppConstants.ExtentName}{viewport.Handle}";
                ...
                using (Curve polyline = viewport.GetViewportCurveInModel(tr, true, out double height, out double width, out _, out _))
                {
                    ...
                    {
                        entity.XDataSet(AppConstants.LayoutCreateAppName, new List<TypedValue> { new TypedValue(Convert.ToInt32(DxfCode.ExtendedDataHandle), viewport.Handle) }, true);
                        BlockReference reference = entity as BlockReference;
                        if (reference != null)
                        {
                            reference.BlockReferenceSetAttribute(tr);
                            reference.BlockReferenceChangeAttribute(tr, new Dictionary<string, string> { { AppConstants.BlockReferenceNumber, i++.ToString() } });
                        }
                    }
        ...
        private static ObjectId GetExtentBlockId(string name, Transaction tr, double width, double height, Curve polyline)
        {
            ...
            Support.CreateLayer(AppConstants.NoPlotLayer, false, tr);
            ...
            //направления
            CreateLine(AppConstants.NoPlotLayer, 0, 0, 0, height / 2, btr, tr, 1);
            CreateLine(AppConstants.NoPlotLayer, 0, 0, width / 2, 0, btr, tr, 1);

            AttributeDefinition attribute = new AttributeDefinition()
            {
                Tag = AppConstants.BlockReferenceNumber,
                ColorIndex = 1,
                Height = width / 5,
                ...
                Preset = false,
                TextString = "",
                Layer = AppConstants.NoPlotLayer,
